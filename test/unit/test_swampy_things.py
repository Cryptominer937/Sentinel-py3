...
def test_valid_swamp_address():
    from swamplib import is_valid_swamp_address

    main = valid_swamp_address()
    test = valid_swamp_address('testnet')

    assert is_valid_swamp_address(main) is True
    assert is_valid_swamp_address(main, 'mainnet') is True
    assert is_valid_swamp_address(main, 'testnet') is False

    assert is_valid_swamp_address(test) is False
    assert is_valid_swamp_address(test, 'mainnet') is False
    assert is_valid_swamp_address(test, 'testnet') is True


def test_invalid_swamp_address():
    from swamplib import is_valid_swamp_address

    main = invalid_swamp_address()
    test = invalid_swamp_address('testnet')

    assert is_valid_swamp_address(main) is False
    assert is_valid_swamp_address(main, 'mainnet') is False
    assert is_valid_swamp_address(main, 'testnet') is False

    assert is_valid_swamp_address(test) is False
    assert is_valid_swamp_address(test, 'mainnet') is False
    assert is_valid_swamp_address(test, 'testnet') is False
...
def test_hash_function():
    ...
    hex_hash = "%x" % swamplib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(swamplib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(swamplib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(swamplib.blocks_to_seconds(16616)) == 2612035
